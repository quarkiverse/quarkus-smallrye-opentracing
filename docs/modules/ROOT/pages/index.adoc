...
If you are interested in maintaining this extension, please reach out to us in the https://github.com/quarkiverse/quarkus-smallrye-opentracing/issues[GitHub issues] of this extension.
====
...
* Apache Maven 3.9.5
* A working container runtime (Docker or https://quarkus.io/guides/podman[Podman])
* The https://quarkus.io/guides/cli-tooling[Quarkus CLI]
* Optionally Mandrel or GraalVM installed and https://quarkus.io/guides/building-native-image#configuring-graalvm[configured appropriately] if you want to build a native executable (or Docker if you use a native container build)
...
The completed example is located in the `quickstart` https://github.com/quarkiverse/quarkus-smallrye-opentracing/tree/main/quickstart[directory].
...
If you followed the https://quarkus.io/guides/kafka[Kafka guide], the interceptors can be added on the `generated-price` and the `prices` channels as follows:
...
Following the https://quarkus.io/guides/mongodb[MongoDB guide], the command listener will be registered defining the config property as follows:
...
include::includes/quarkus-smallrye-opentracing.adoc[leveloffset=+1, opts=optional]

== Jaeger Configuration Reference

For more information about the Jaeger extension configuration, have a look at the https://docs.quarkiverse.io/quarkus-jaeger/dev/index.html[Jaeger extension documentation].
